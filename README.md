...
**Unfinished.** Version 0.1.6. No support.
...
2. Unpack it anywhere you want (Documents, a tools folder, a drive of your choice).
   The zip has a version folder; put `run.cmd` at the top of your chosen folder.
3. If `runtime\python\python.exe` is not already there, fetch the pinned embeddable
   runtime into this folder once:

```text
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\fetch-runtime.ps1
```

That is the normal Python path. You do not need a separate python.org install.
If you already have Python 3.11+ on PATH, `run.cmd` will use it when the
bundled runtime is missing. See [No Python?](#no-python) only if both fail.
...
Only app code is replaced (`instance_mod_updater\`, `scripts\`, `tests\`,
`run.cmd`, `run.ps1`, `deploy.cmd`, docs, and so on).

Left alone: `runtime\`, `jars\`, `manifest.json`, `report-*`, `pack-*.json`,
...
`run.cmd` looks for `runtime\python\python.exe` first, then a real Python 3.11+
on PATH (`py` or `python`, not the Microsoft Store stub). If none is found:

```text
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\fetch-runtime.ps1
```

Then run `.\run.cmd list` again. Installing python.org 3.11+ with PATH enabled
is only a fallback if you prefer a system interpreter.

## From source (optional)

Git clone into any folder if you prefer that over the Release zip.
You still need the fetch-runtime step (or system Python). Prefer the Release zip
for normal use; `main` is unsigned.
...
instance-mod-updater/
  ...
  deploy.cmd
  README.md
  runtime\          (created by fetch-runtime.ps1; not in the Release zip)
  scripts/
    fetch-runtime.ps1
    self-update.ps1
    sign-release.py
